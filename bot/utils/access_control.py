...
def get_allowed_users() -> List[int]:
    ...
    if not allowed_users_str:
        logger.warning("ALLOWED_USERS environment variable is not set. No users will be allowed to access the bot.")
        return []
    
    try:
        # Split by comma and convert to integers
        allowed_users = [int(user_id.strip()) for user_id in allowed_users_str.split(",") if user_id.strip()]
        logger.info(f"Loaded {len(allowed_users)} allowed users from environment variable")
        return allowed_users
    except ValueError as e:
        logger.error(f"Error parsing ALLOWED_USERS environment variable: {e}. Format should be comma-separated integers.")
        return []
