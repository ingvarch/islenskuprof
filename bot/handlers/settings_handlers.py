...
from bot.db.user_service import (
    ...
    get_all_languages,
    get_all_audio_speeds,
    update_user_audio_speed,
    get_all_language_levels,
    update_user_language_level,
    get_all_target_languages,
    get_target_language_by_id,
    update_user_target_language,
    get_user_background_effects,
    update_user_background_effects,
)
...
async def settings_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "🌍 " + get_translation("bot_language", language),
                callback_data=LANGUAGE_MENU,
            ),
            InlineKeyboardButton(
                "📚 " + get_translation("target_language", language),
                callback_data=TARGET_LANGUAGE_MENU,
            ),
        ],
        [
            InlineKeyboardButton(
                "📝 " + get_translation("learning_level", language),
                callback_data=LANGUAGE_LEVEL_MENU,
            ),
            InlineKeyboardButton(
                "🎧 " + get_translation("voice_speed", language),
                callback_data=AUDIO_SPEED_MENU,
            ),
        ],
        [
            InlineKeyboardButton(
                f"🔊 {get_translation('background_effects', language)} {bg_effects_display}",
                callback_data=BACKGROUND_EFFECTS_MENU,
            )
        ],
    ]
    ...
    logger.info(
        "Creating settings keyboard with language, target language, language level, voice speed, and background effects buttons"
    )
...
async def language_menu_callback(
    ...
    if not languages:
        logger.error(
            "No languages found in database. Make sure migrations have been run."
        )
    ...
    for language in languages:
        ...
        logger.info(
            f"Adding language button: {button_text} with callback_data: {callback_data}"
        )
        keyboard.append(
    ...
    try:
        ...
        logger.info(
            f"Successfully edited message to show language selection for user {user.id}"
        )
    except Exception as e:
        logger.error(f"Error editing message: {e}", exc_info=True)


async def language_select_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    if success:
        ...
        try:
            ...
            logger.info(
                f"Successfully updated message with confirmation for user {user.id}"
            )
        except Exception as e:
            logger.error(f"Error editing message: {e}", exc_info=True)
    else:
...
async def audio_speed_menu_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    if not audio_speeds:
        logger.error(
            "No audio speeds found in database. Make sure migrations have been run."
        )
    ...
    if (
        hasattr(db_user, "settings")
        and db_user.settings
        and db_user.settings.audio_speed
    ):
    ...
    for audio_speed in audio_speeds:
        ...
        logger.info(
            f"Adding audio speed button: {button_text} with callback_data: {callback_data}"
        )
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    logger.info(
        f"Created audio speed selection keyboard with {len(audio_speeds)} buttons"
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully edited message to show audio speed selection for user {user.id}"
        )
    except Exception as e:
        logger.error(f"Error editing message: {e}", exc_info=True)


async def audio_speed_select_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    if success:
        ...
        message = (
            get_translation(
                "audio_speed_updated", language, speed=audio_speed_description
            )
            + " ✅"
        )
        ...
        try:
            ...
            await query.edit_message_text(
                text=get_translation(
                    "error_occurred",
                    "English",
                    error="Failed to update audio speed preference",
                )
            )
        except Exception as e:
            logger.error(f"Error editing message: {e}", exc_info=True)
...
async def language_level_menu_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    if not language_levels:
        logger.error(
            "No language levels found in database. Make sure migrations have been run."
        )
        await query.answer(text=get_translation("no_languages", language))
        await query.edit_message_text(
            text=get_translation("error_no_languages", language)
    ...
    if (
        hasattr(db_user, "settings")
        and db_user.settings
        and db_user.settings.language_level
    ):
        current_language_level_id = db_user.settings.language_level.id
        logger.info(
            f"User {user.id} current language level ID: {current_language_level_id}"
        )
    ...
    for language_level in language_levels:
        ...
        logger.info(
            f"Adding language level button: {button_text} with callback_data: {callback_data}"
        )
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    logger.info(
        f"Created language level selection keyboard with {len(language_levels)} buttons"
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully edited message to show language level selection for user {user.id}"
        )
    except Exception as e:
        logger.error(f"Error editing message: {e}", exc_info=True)


async def language_level_select_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    if success:
        ...
        # Confirmation message using translation
        message = (
            get_translation("language_level_updated", language, level=language_level)
            + " ✅"
        )
        ...
        try:
            ...
            await query.edit_message_text(
                text=get_translation(
                    "error_occurred",
                    "English",
                    error="Failed to update language level preference",
                )
            )
        except Exception as e:
            logger.error(f"Error editing message: {e}", exc_info=True)
...
async def target_language_menu_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    if not target_languages:
        logger.error(
            "No target languages found in database. Make sure migrations have been run and data seeded."
        )
        await query.answer(text=get_translation("no_languages", language))
        await query.edit_message_text(
    ...
    if (
        ...
        and hasattr(db_user, "settings")
        and db_user.settings
        and db_user.settings.target_language_id
    ):
        current_target_language_id = db_user.settings.target_language_id
        logger.info(
            f"User {user.id} current target language ID: {current_target_language_id}"
        )
    ...
    for target_lang in target_languages:
        ...
        logger.info(
            f"Adding target language button: {button_text} with callback_data: {callback_data}"
        )
        keyboard.append(
            [InlineKeyboardButton(button_text, callback_data=callback_data)]
        )
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    logger.info(
        f"Created target language selection keyboard with {len(target_languages)} buttons"
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully edited message to show target language selection for user {user.id}"
        )
    except Exception as e:
        logger.error(f"Error editing message: {e}", exc_info=True)


async def target_language_select_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    if success:
        ...
        message = (
            get_translation(
                ...
                language,
                lang_name=f"{target_lang_native} ({target_lang_name})",
            )
            + " ✅"
        )
        ...
        try:
            ...
            await query.edit_message_text(
                text=get_translation(
                    "error_occurred",
                    "English",
                    error="Failed to update target language preference",
                )
            )
        except Exception as e:
            logger.error(f"Error editing message: {e}", exc_info=True)
...
async def background_effects_menu_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
...
async def background_effects_select_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    if success:
        ...
        message = (
            get_translation(
                "background_effects_updated", language, status=preset_display
            )
            + " ✅"
        )
        ...
        try:
            await query.edit_message_text(
                text=get_translation(
                    "error_occurred",
                    language,
                    error="Failed to update background effects",
                )
            )
        except Exception as e:
            logger.error(f"Error editing message: {e}", exc_info=True)
...
async def settings_callback_handler(
    ...
    if callback_data == LANGUAGE_MENU:
        ...
        await query.edit_message_text(
            text=get_translation(
                "error_occurred", language, error=f"Unknown action: {callback_data}"
            )
        )
