...
def upgrade():
    logger.info(
        "Applying migration %s: Adding custom lesson settings columns", revision
    )
    ...
    op.add_column(
        "pimsleur_custom_lessons",
        sa.Column(
            "voice_preference", sa.String(10), nullable=False, server_default="both"
        ),
    )
    op.add_column(
        "pimsleur_custom_lessons",
        sa.Column(
            "difficulty_level", sa.String(5), nullable=False, server_default="auto"
        ),
    )
    op.add_column(
        "pimsleur_custom_lessons",
...
def downgrade():
    logger.info(
        "Rolling back migration %s: Removing custom lesson settings columns", revision
    )
