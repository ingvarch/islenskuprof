...
def upgrade():
    logger.info(
        "Applying migration %s: Adding audio_speeds and user_settings tables", revision
    )
    ...
    op.create_foreign_key(
        "fk_user_settings_user_id", "user_settings", "users", ["user_id"], ["id"]
    )
...
def downgrade():
    logger.info(
        "Rolling back migration %s: Removing user_settings and audio_speeds tables",
        revision,
    )

    # Remove foreign key constraints
    op.drop_constraint(
        "fk_user_settings_audio_speed_id", "user_settings", type_="foreignkey"
    )
