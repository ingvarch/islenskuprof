...
def upgrade():
    logger.info(
        "Applying migration %s: Adding language_levels table and language_level_id to user_settings",
        revision,
    )
    ...
    op.bulk_insert(
        sa.table("language_levels", sa.column("level", sa.String)),
        [
            {"level": "A1"},
            {"level": "A2"},
            {"level": "B1"},
    ...
    op.add_column(
        "user_settings", sa.Column("language_level_id", sa.Integer, nullable=True)
    )
...
def downgrade():
    logger.info(
        "Rolling back migration %s: Removing language_level_id from user_settings and dropping language_levels table",
        revision,
    )

    # Remove foreign key constraint
    op.drop_constraint(
        "fk_user_settings_language_level_id", "user_settings", type_="foreignkey"
    )
