...
def upgrade():
    logger.info("Applying migration %s: Adding background_effects to user_settings", revision)

    # Add background_effects column to user_settings table with default False
    op.add_column('user_settings', sa.Column('background_effects', sa.Boolean, nullable=False, server_default='0'))

    logger.info("Migration %s applied successfully", revision)


def downgrade():
    logger.info("Rolling back migration %s: Removing background_effects from user_settings", revision)
