...
def upgrade():
    logger.info(
        "Applying migration %s: Adding language_id to user_settings table", revision
    )
...
def downgrade():
    logger.info(
        "Rolling back migration %s: Removing language_id from user_settings table",
        revision,
    )

    # Remove foreign key constraint
    op.drop_constraint(
        "fk_user_settings_language_id", "user_settings", type_="foreignkey"
    )
