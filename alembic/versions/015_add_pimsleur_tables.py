...
def upgrade():
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('language_code', 'level', 'lesson_number', name='uq_pimsleur_lesson')
    )
    ...
    op.create_table(
        ...
        sa.Column('introduced_lesson_id', sa.Integer, sa.ForeignKey('pimsleur_lessons.id', ondelete='SET NULL'), nullable=True),
        sa.Column('frequency_rank', sa.Integer, nullable=True),
        sa.Column('audio_file_path', sa.String(500), nullable=True),
        sa.Column('created_at', sa.DateTime, server_default=sa.func.now()),
        sa.UniqueConstraint('language_code', 'word_target', name='uq_pimsleur_vocab_word')
    )
    ...
    op.create_table(
        ...
        sa.Column('id', sa.Integer, primary_key=True, autoincrement=True),
        sa.Column('lesson_id', sa.Integer, sa.ForeignKey('pimsleur_lessons.id', ondelete='CASCADE'), nullable=False),
        sa.Column('vocabulary_id', sa.Integer, sa.ForeignKey('pimsleur_vocabulary.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('last_lesson_id', sa.Integer, sa.ForeignKey('pimsleur_lessons.id', ondelete='SET NULL'), nullable=True),
        ...
        sa.Column('created_at', sa.DateTime, server_default=sa.func.now()),
        sa.Column('updated_at', sa.DateTime, server_default=sa.func.now(), onupdate=sa.func.now()),
        sa.UniqueConstraint('user_id', 'language_code', name='uq_user_pimsleur_progress')
    )
    ...
    op.create_index('ix_pimsleur_lessons_lookup', 'pimsleur_lessons', ['language_code', 'level', 'lesson_number'])
    op.create_index('ix_user_pimsleur_progress_user', 'user_pimsleur_progress', ['user_id', 'language_code'])
    op.create_index('ix_pimsleur_custom_lessons_user', 'pimsleur_custom_lessons', ['user_id', 'status'])
...
def downgrade():
    ...
    # Drop indexes
    op.drop_index('ix_pimsleur_custom_lessons_user', table_name='pimsleur_custom_lessons')
